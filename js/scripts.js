...
const htmlStarCreator = bookID => {
	...
	for (let i = 5; i >= 0.5; i -= 0.5){
		innerHTML += `<input type="radio" id="${bookID}rating-${i}" name="rating" value="${i}">
...
const ifRatingBlank = dataPath => dataPath ? dataPath.map(item => (item.type === "ISBN_13") ? item.identifier : "" ) : "";
...
const render = book => {
  ...
  document.getElementById(`${myLibrary.indexOf(book)}rating-${book.myRating}`).checked = true;
